...
function collectionPage({ darkMode }: DarkModeProps) {
    ...
    return (
        ...
        className={`relative bg-lightMode-background dark:bg-darkMode-colors-background ${
            darkMode ? "dark" : ""
        }  grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 justify-center gap-7 py-8 px-5 dark:bg-darkMode-colors-background duration-200`}
        >
        {collectionsData.map((collection, idx) =>
            ...
            <Link
                ...
                id: collection.id,
                image:
                    collection.preview_photos[1].urls.raw +
                    "&w=180&h=300&fit=max&dpr=1",
